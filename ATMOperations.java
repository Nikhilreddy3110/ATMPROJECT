...
import com.codegnan.customExceptions.InvaildCardException;
import com.codegnan.customExceptions.InvaildPinException;
...
public class ATMOperations {
	...
	public static IATMService validateCard(long cardNumber) throws InvaildCardException {
		if (dataBase.containsKey(cardNumber)) {
			return dataBase.get(cardNumber);
		} else {
			ACTIVITY.add("Accessed by: " + cardNumber + " is Not Compatible");
			throw new InvaildCardException("This is Not A valid Card");
		}
	}
	...
	public static void resetUserAttempts(IATMService operatorCard) {
		...
		} catch (InvaildCardException ive) {
	...
	public static IATMService validateCredentials(long cardNumber, int pinNumber)
			throws InvaildCardException, IncorrectPinLimitReachedException, InvaildPinException {
		if (dataBase.containsKey(cardNumber)) {
			card = dataBase.get(cardNumber);
		} else {
			throw new InvaildCardException("This card is Not A valid Card");
		}
		try {
			if (card.getUserType().equals("operator")) {
				// Operators have a different PIN validation process
				if (card.getPinNumber() != pinNumber) {
					throw new InvaildPinException("Dear operator, Please enter Correct PIN Number");
				} else {
					return card;
				}
			}
		...
		if (card.getPinNumber() != pinNumber) {
			...
			throw new InvaildPinException("You Have Entered A Wrong PIN Number");
		} else {
			return card;
		}
	}
	...
	public static void validateDepositAmount(double amount)
			throws InsufficentMachineBalanceException, InvaildAmountException {
		// Ensure deposit amount is a multiple of 100
		if (amount % 100 != 0) {
			throw new InvaildAmountException("Please deposit amounts in multiples of 100.");
		}
		...
		while (flag) {
			...
			case 2:
				...
				try {
					...
					System.out.println("==================================================================");
				} catch (InvaildAmountException | InsufficentMachineBalanceException e) {
		...
		while (MACHINE_ON) {
			...
			try {
				...
				while (true) {
					...
					} catch (InvaildAmountException | InsufficientBalanceException
							| InsufficentMachineBalanceException e) {
						System.out.println(e.getMessage());
					}
				}
			} catch (InvaildPinException | InvaildCardException | IncorrectPinLimitReachedException e) {
